...
def get_ticket_data(ticker):
    ...
    for row in soup.select('.snapshot-table2 tr'):
        ...
        sector_table = soup.find("table",{"class":"fullview-title"})
        data['Sector'] = sector_table.text.split('|')[1].split('\n')[-1].strip()
        data['Industry'] = sector_table.text.split('|')[2].strip()  
